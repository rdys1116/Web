...
import { fetchData, getFullUrl, getWatchedMedia } from '../../../frontend/src/common-functions/functions';
...
function useFetchPageDetails(){
    ...
    useEffect(() => {
        fetchData(getFullUrl(pageNumber, 'tv/60735'), setBackGroundPoster, backGroundPoster);
        fetchData(getFullUrl(pageNumber, 'movie/popular'), setPopularMovies, PopularMovies);
        fetchData(getFullUrl(pageNumber, 'movie/top_rated'), setTopRatedMovies, TopRatedMovies);
        fetchData(getFullUrl(pageNumber, 'tv/popular'), setPopularTvShow, PopularTvShow);
        fetchData(getFullUrl(pageNumber, 'tv/top_rated'), setTopRatedTvShow, TopRatedTvShow);
    ...
    function loadMoreOnClick(e){
        ...
        if(name === 'movie/popular'){
            ...
            pgPopularMovie =+ 1;
            fetchData(getFullUrl(pgPopularMovie, name), setPopularMovies, PopularMovies);

        } else if(name === 'movie/top_rated'){
        
            pgTopRatedMovies =+ 1;
            fetchData(getFullUrl(++pgTopRatedMovies, name), setTopRatedMovies, TopRatedMovies);
        
        } else if(name === 'tv/popular'){
            
            pgPopularTvShow =+ 1;
            fetchData(getFullUrl(++pgPopularTvShow, name), setPopularTvShow, PopularTvShow);
        
        } else if(name === 'tv/top_rated'){
        
            pgTopRatedTvShow =+ 1;
            fetchData(getFullUrl(++pgTopRatedTvShow, name), setTopRatedTvShow, TopRatedTvShow);
